...
// Input: String represening operations and integers.
// Output: Register values printed when

// Algorithim:
// Split array on ' '.
// Iterate through and handle commands as such:
// if Number, set register equal to value.
// PUSH: push register value into stack. leaving in register.
//ADD : Pop a value from the stack and add it to the register value, storing the result in the register.
//SUB : Pop a value from the stack and subtract it from the register value, storing the result in the register.
//MULT : Pop a value from the stack and multiply it by the register value, storing the result in the register.
//DIV : Pop a value from the stack and divide it into the register value, storing the integer result in the register.
...
function minilang(string) {
  ...
  commands.forEach(command => {
    if (command.match(/(ADD|SUB|MULT|DIV|MOD|POP)/) && stack.length === 0) {
      throw new Error('The stack is empty!');
    }
    if (!command.match(/(ADD|SUB|MULT|DIV|MOD|POP|PUSH|POP|PRINT|\d+)/)) {
